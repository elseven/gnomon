		...
		static public UIWidget.Pivot[] pivotPoints =
	{
		...
		protected override void DrawCustomProperties ()
		{
				if (NGUISettings.unifiedTransform) {
						...
						DrawInspectorProperties (serializedObject, mWidget, true);
		}
		...
		public void DrawWidgetTransform ()
		{
				DrawInspectorProperties (serializedObject, mWidget, false);
		}
		...
		static UIWidget.Pivot Combine (UIWidget.Pivot horizontal, UIWidget.Pivot vertical)
		{
				if (horizontal == UIWidget.Pivot.Left) {
						if (vertical == UIWidget.Pivot.Top)
								return UIWidget.Pivot.TopLeft;
						if (vertical == UIWidget.Pivot.Bottom)
		...
		{
				if (mAnchorType == AnchorType.Advanced || !mWidget.isAnchored)
						return;
